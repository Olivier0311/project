...
<?php
/**
 * Strongly inspired by bin/php/ezasynchronouspublisher.php
...
/**
 * ezacceleratorasynchronouscleaner : Varnish Purge daemon
 *
...
$cli = eZCLI::instance();
$script = eZScript::instance( array( 	'description' => "Processes the eZ Accelerator Purge in queue",
										'use-session' => false,
										'use-modules' => true,
										'use-extensions' => true ) );
...
else
{
	...
	$pidFile = "$varDir/run/ezacceleratorasynchronouspurger.pid";
}
...
if ( $options['daemon'] )
{
	...
	$pid = getmypid();
	$cli->output( "eZ Accelerator daemon started. Process ID: $pid" );
	fputs( $pidFp, $pid );
...
// actual daemon code
$processor = eZAcceleratorPurgingQueueProcessor::instance();
$processor->setCli( $cli, isset($options['daemon']));
$processor->run();
...
function daemonSignalHandler( $signo )
{
	switch( $signo )
	{
		case SIGTERM:
			...
			eZAcceleratorPurgingQueueProcessor::terminate();
